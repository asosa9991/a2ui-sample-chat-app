...
| 2026-04-18 | intent_router robustness: wrap each template's processing in try/except Exception; check isinstance(template, dict) before .get(); validate each kw in exact phrase is str before .lower(); existing isinstance(token, str) check for keywords was already correct |
| 2026-04-18 | designer/save-template 409 guard: check tfile.exists() AND _is_template_approved(req.templateId) before write; if True raise HTTPException(409); draft (status="draft") allows overwrite |
| 2026-04-18 | transform_to_path_bindings generic: scan ALL immediate keys in widget config for {literalString: ...}; path_key = comp_id when key=="text" (backward compat), else f"{comp_id}_{key}"; no deep recursion needed |
| 2026-04-18 | TestDesignerSaveTemplate uses TestClient(app, raise_server_exceptions=False); autouse fixture cleans up test draft files before/after each test; _minimal_request() helper builds valid SaveTemplateRequest payload |
